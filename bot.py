...
async def start(update: Update, context: ContextTypes):
    ...
    button = [[KeyboardButton(text="💭кумариться с ботом - общаться")], [KeyboardButton(text="🌄Генерация картинок — кал полнейший")], [KeyboardButton(text="🌅Генерация картинок — легенда Stable Diffusion")]]
    ...
    if not result:
        ...
        await update.message.reply_text(
            "👋У вас есть: \n💭3000 токенов для текста \n🌄3 шакальных картинок dall-e \n🌅3 шедевро стабле-дифузион\n Выберите опцию: 👇 \n если кнопки не работают - наишите /start",
...
async def pre_chatgpt_answer_handler(update: Update, context: ContextTypes):
    ...
    if result > 0:
        ...
        if answer != None:
            ...
            await update.message.reply_text(
                "❌Ваш запрос активировал систему защиты API и не может быть обработан. Пожалуйста, измените запрос и попробуйте снова.",
                reply_markup=reply_markup,
            )
        
    else:
        await update.message.reply_text(
            "❎You have 0 tokens. You need to buy them to use ChatGPT.",
            reply_markup=reply_markup,
            )
...
async def display_info(update: Update, context: ContextTypes):
    ...
    await update.message.reply_text(
        f"У вас: \n 💭{result[2]} токенов для текста \n 🌄{result[3]} DALL·E генераций картинок \n 🌅{result[4]} Stable Diffusion генераций картинок",
        reply_markup=reply_markup,
        )
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            ENTRY_STATE: [
                ...
                MessageHandler(filters.Regex('^💭Общение с ботом$'), pre_chatgpt_handler),
                MessageHandler(filters.Regex('^🌄Шакальная генерация картинок — DALL·E$'), pre_dall_e_handler),
                MessageHandler(filters.Regex('^🌅Шедевро генерация картинок — Stable Diffusion$'), pre_stable_handler),
            ...
            PURCHASE_STATE: [
                CommandHandler('start', start),
                MessageHandler(filters.Regex('^🔙Назад$'), display_info),
